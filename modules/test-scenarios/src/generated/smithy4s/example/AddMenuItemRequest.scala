...
object AddMenuItemRequest extends ShapeTag.Companion[AddMenuItemRequest] {
  ...
  private def make(restaurant: String, menuItem: MenuItem): AddMenuItemRequest =
    AddMenuItemRequest(restaurant, menuItem)

  implicit val schema: Schema[AddMenuItemRequest] = struct(
    string
      .required[AddMenuItemRequest]("restaurant", _.restaurant)
      .addHints(smithy.api.HttpLabel()),
    MenuItem.schema
      .required[AddMenuItemRequest]("menuItem", _.menuItem)
