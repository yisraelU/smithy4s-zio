  ...
  def flatMapErrors(
    ...
    new ServerEndpointMiddleware {
      def prepare[Alg[_[_, _, _, _, _]]](service: Service[Alg])(
        ...
        val handler: Handler[Any, Throwable, (zio.http.Path, zio.http.Request), zio.http.Response] =
          Handler.fromFunctionZIO { (pathAndRequest: (zio.http.Path, zio.http.Request)) =>
