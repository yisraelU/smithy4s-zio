...
final class SchemaVisitorZSchemaGen(
  ...
  override def struct[S](
    ...
    // Create ZIO Schema fields for the GenericRecord
  ...
  override def union[U](
    ...
    // Compile schemas for each alternative (stored for decoding)
    ...
    def encode(u: U): (String, DynamicValue) = {
      ...
      alternatives.foreach { alt =>
        alt.project.lift(u).foreach { a =>
          ...
          val dynValue = DynamicValue.fromSchemaAndValue(schema.asInstanceOf[ZSchema[Any]], a)
