package smithy4s.zio.transformers
...
final class SimpleRestJsonProtocolTransformer extends ProjectionTransformer {
  ...
  println(s"loading the transformer now at ${System.currentTimeMillis()}")
        ...
        theTrait match {
          ...
          case c: HttpRequestTestsTrait =>
            new HttpRequestTestsTrait(
              c.getSourceLocation,
              c.getTestCases.asScala.toList.map {
                case req: HttpRequestTestCase =>
                  if (
                    req.getProtocol == ShapeId.from("aws.protocols#restJson1")
                  ) {
